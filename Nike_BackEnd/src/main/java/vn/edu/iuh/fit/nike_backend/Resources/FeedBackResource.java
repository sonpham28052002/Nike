...
public class FeelBackResource {
    ...
    public Feedback getFeelback(@PathVariable long id, @PathVariable long user_id, @PathVariable long product_id){
    ...
    public boolean insertFeelBack(@PathVariable long id,@PathVariable long user_id, @PathVariable long product_id , @RequestBody Feedback feelback){
        User user = userRepository.findById(user_id).get();
        Product product = productRepository.findById(product_id).get();
